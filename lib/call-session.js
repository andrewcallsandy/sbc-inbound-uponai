...
class CallSession extends Emitter {
  ...
  async connect() {
    ...
    this.logger.info(`inbound ${is3pcc ? '3pcc ' : ''}call accepted for routing - FROM: ${this.req.callingNumber} TO: ${this.req.calledNumber}`);
    ...
    try {
      ...
      this.logger.info(`call connected successfully to feature server - FROM: ${this.req.callingNumber} TO: ${this.req.calledNumber}`);
...
Duration=${payload.duration} `
  ...
  async _onFeatureServerTransfer(dlg, req, res) {
    try {
      ...
      if (!arr) {
        ...
        if ([401, 407].includes(response.status)) {
          const retryStatus = await this._handleReferAuthChallenge(
            response, uri, referHeaders
          );
          if (retryStatus !== null) return res.send(retryStatus);
        }
  ...
  async _handleReferAuthChallenge(response, uri, referHeaders) {
    const wwwAuth = response.status === 407
      ? response.get('Proxy-Authenticate')
      : response.get('WWW-Authenticate');
    if (!wwwAuth) {
      this.logger.info('_handleReferAuthChallenge: no auth challenge header found');
      return null;
    }

    const {gateway} = this.req.locals;
    if (!gateway?.voip_carrier_sid) {
      this.logger.info('_handleReferAuthChallenge: no voip_carrier_sid available');
      return null;
    }

    const creds = await this.srf.locals.getCarrierCredentials(gateway.voip_carrier_sid);
    if (!creds) {
      this.logger.info({voip_carrier_sid: gateway.voip_carrier_sid},
        '_handleReferAuthChallenge: no credentials configured for carrier');
      return null;
    }

    try {
      const challenge = this._parseDigestChallenge(wwwAuth);
      const digestUri = `sip:${stringifyUri(uri)}`;
      const authHeader = this._computeDigestResponse(
        creds.username, creds.password, challenge, 'REFER', digestUri
      );

      const headerName = response.status === 407 ? 'Proxy-Authorization' : 'Authorization';
      this.logger.info({headerName, realm: challenge.realm},
        '_handleReferAuthChallenge: retrying REFER with Digest credentials');

      const retryResponse = await this.uas.request({
        method: 'REFER',
        headers: {
          ...referHeaders,
      ...
      this.logger.info({status: retryResponse.status},
        '_handleReferAuthChallenge: REFER retry response');
      return retryResponse.status;
    } catch (err) {
      this.logger.error(err, '_handleReferAuthChallenge: error computing or sending auth');
      return null;
    }
  }
  ...
  _computeDigestResponse(username, password, challenge, method, digestUri) {
    ...
    if (qop === 'auth' || qop === 'auth-int') {
      ...
      authLine = `Digest username="${username}", realm="${realm}", nonce="${nonce}", ` +
        `uri="${digestUri}", response="${response}", algorithm=${algo}, ` +
