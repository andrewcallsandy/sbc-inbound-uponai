...
const sqlSelectCarrierCredentials = `
SELECT register_username, register_password
FROM voip_carriers
WHERE voip_carrier_sid = ?
AND register_username IS NOT NULL
AND register_password IS NOT NULL`;
...
module.exports = (srf, logger) => {
  ...
  const getCarrierCredentials = async(voip_carrier_sid) => {
    try {
      const [r] = await pp.query(sqlSelectCarrierCredentials, [voip_carrier_sid]);
      if (0 === r.length) return null;
      return {username: r[0].register_username, password: r[0].register_password};
    } catch (err) {
      logger.error({err}, 'getCarrierCredentials');
      return null;
    }
  };
        ...
        */
        ...
        if (accountLevelGateways.length > 1) {
          logger.info({accounts: accountLevelGateways.map((m) => m.account_sid)},
            `multiple accounts have added this carrier with default routing -- cannot determine which to use - FROM: ${req.callingNumber} TO: ${req.calledNumber}`);
          return {
            fromCarrier: true,
